...
export type RouteSeoConfig = {
  ...
  robots: string;
  ogImage?: string;
  jsonLdType?: ("WebSite" | "WebPage" | "FAQPage" | "EducationalOrganization" | "BreadcrumbList")[];
};
...
export const seoConfig: Record<string, RouteSeoConfig> = {
  "/": {
    ...
    robots: "index,follow",
    jsonLdType: ["WebSite", "EducationalOrganization"],
  },
  "/industry-overview": {
...
export const buildBreadcrumbJsonLd = (path: string) => {
  const items = guideNavItems.filter(i => i.path === "/" || i.path === path);
  return {
